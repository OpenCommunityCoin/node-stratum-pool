...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        let headerHex = headerBuffer.toString('hex');
        ...
        if (job.target.ge(headerBigNum)){
            ...
            // previous blockHash
            previousBlockHash = util.reverseBuffer(new Buffer(headerHex.substr(8, 64), 'hex')).toString('hex');
            ...
            sha256dBlockHash = util.reverseBuffer(util.sha256d(headerBuffer)).toString('hex');
            console.log("--------------- After job.target.ge(headerBigNum) is true,  blockHex / blockHash -----------");
            console.log(blockHex);
            console.log(blockHash);
            console.log('sha256d blockHash = ', sha256dBlockHash);
            console.log("--------------------------------------------------------------------------------------------");
        }
        else {
