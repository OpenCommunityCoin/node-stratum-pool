...
var pool = module.exports = function pool(options, authorizeFn){
            ...
            DetectCoinData(function(){
                ...
                OnBlockchainSynced(function(){
                    GetFirstJob(function(){
                         ...
                         * If 'reward' key value of hdac.json is ePoW, apply redis pub/sub.
                         */
    ...
    function GetFirstJob(finishedCallback){
        ...
        /**
         * @HDAC
         * It sets the findBlocks and blockwindowsize of the Hdac core when running the first server.
         */
        GetBlockWindowSize(function(result){
            ...
            if(isSuccess) {
                ...
                sharedData.setBlockWindowSize(0);
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var generateProgress = function(){
            ...
            function(error, blockCount, callback) {
            ...
            */
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    // previous blockHash
                    shareData.blockHash = shareData.previousBlockHash;

                    CheckBlockAccepted(shareData, blockHex, this._daemon, function(isAccepted, tx, height, blockhash) {
                        isValidBlock = isAccepted;

                        // store blockhash, tx, height
                        shareData.blockHash = blockhash;
                        shareData.txHash = tx;
                        shareData.height = height;
                        ...
                        GetBlockTemplate(function(error, result, foundNewBlock){
                            if (foundNewBlock) {
                                /*
                                 * @HDAC
                                 * If you find the block, publish it. message propagate the height to will be found.
                                 * if blockwindowsize is 0, passed it.
                                 */
                                console.log("================================find block !!!!!!!!!!!!!!!!!!!!!!!!!!  ========================================")
                                if(sharedData.getBlockWindowSize() > 0) {
                                    var findBlocks = result.height;
                                    publisher.publish(redisChannels[0], findBlocks);
                                }
                                console.log("================================================================================================================")
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.testnet = rpcResults.getinfo.testnet || (rpcResults.getblockchaininfo && rpcResults.getblockchaininfo.chain != "main");
            ...
            options.initStats = {
                ...
                difficulty: (rpcResults.getinfo.difficulty || rpcResults.getblockchaininfo.difficulty) * algos[options.coin.algorithm].multiplier,
    ...
    function StartStratumServer(finishedCallback){
        ...
        _this.stratumServer.on('started', function(){
            ...
            /*
             * @HDAC
             * Register Listener related ePoW
             */
            }).on('releaseEpow', function(){
                emitLog('Release ePoW ' + client.remoteAddress);

            }).on('keepOnEpow', function(){
                emitLog('Keep On ePoW ' + client.remoteAddress);
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                if (result.error){
                    ...
                    /*
                     * @HDAC
                     * Every time get information through the getblocktemplate on the Hdac core, check the height of the Hdac core and set the value to sharedData.
                     */
                    sharedData.setCurrentHeight(result.response.height);
    ...
    function CheckBlockAccepted(shareData, blockHex, daemon, callback){
        var blockHash = shareData.previousBlockHash;
            ...
            _this.daemon.cmd('getblock',
                ...
                function(results){
                    ...
                    if (validResults.length >= 1) {
                        if (!validResults[0].response.nextblockhash) {
                            // this is the latest block
                            cb(null, blockHash);
                            return;
                        } else if (validResults[0].response.nextblockhash === shareData.blockHash) {
                            cb(null, validResults[0].response.nextblockhash);
                            return;
                        } else {
                            cb(null, validResults[0].response.nextblockhash);
                            return;
        ...
        function(newBlockHash, cb) {
            _this.daemon.cmd('getblocktemplate',
                [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}],
                function(results) {
                    if (results[0].error){
                        emitErrorLog('CheckBlockAccepted() - getblocktemplate call failed for daemon instance ' +
                            result.instance.index + ' with error ' + JSON.stringify(results[0].error));
                        cb(results[0].error);
                    } else {
                        cb(null, newBlockHash, results[0].response);
                    }
                }
            );
        }],
        function(error, newBlockHash, response) {
            if (error) {
                return callback(false);
            }
            emitLog('previous block = ' + blockHash + ", last block = " + response.previousblockhash);
            if (newBlockHash === response.previousblockhash) {
                // normal case. getblock.nextblock == getblocktemplate.previousblock
                blockHash = response.previousblockhash;
            } else {
                // use getblock.nextblockhash as submitted block
            ...
            _this.daemon.cmd('getblock',
                ...
                function(results){
                    ...
                    if (validResults.length >= 1){
                        callback(true, validResults[0].response.tx[0], validResults[0].response.height, validResults[0].response.hash);
                    }
                    else{
                        callback(false);
                    }
